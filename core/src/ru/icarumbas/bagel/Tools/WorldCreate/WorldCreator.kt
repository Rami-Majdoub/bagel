...
class WorldCreator(val gameScreen: GameScreen) {
    ...
    var mapWidth = 7.68f
    var mapHeight = 5.12f
    var mapLinks = ArrayList<Array<Int>>()
    ...
    fun createNewWorld() {
        ...
        b2DWorldCreator.loadBodies(map.layers.get("ground"), gameScreen.world, bodyArrays.first(), gameScreen.GROUND_BIT)
        ...
        if (map.layers.get("platform") != null)
            b2DWorldCreator.loadBodies(map.layers.get("platform"), gameScreen.world, platformArrays.first(), gameScreen.PLATFORM_BIT)
    ...
    private fun generateRoom(path: String, side: String, previousMapLink: Int, currentMapLink: Int, count: Int,
        ...
        if (maps[count].properties.get(side) == "Yes") {
            ...
            if (mesh[meshY + closestY][meshX + closestX] == 0) {
                ...
                bodyArrays.add(ArrayList<Body>())
                b2DWorldCreator.loadBodies(maps[currentMap].layers.get("ground"), gameScreen.world, bodyArrays[currentMap], gameScreen.GROUND_BIT)

                // Load platforms for new room
                platformArrays.add(ArrayList<Body>())
                if (maps[currentMap].layers.get("platform") != null)
                    b2DWorldCreator.loadBodies(maps[currentMap].layers.get("platform"), gameScreen.world, platformArrays[currentMap], gameScreen.PLATFORM_BIT)
